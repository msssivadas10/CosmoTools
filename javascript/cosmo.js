...
function cosmologyModule(){
    ...
    function cosmology(cm){
        ...
        /**
         * Returns the baryon density at redshift z in h^2 Msun/Mpc^3. 
         */
         function rho_b(z){

            return Ob0 / Om0 * rho_m(z);

        }

        /**
         * Returns the massive neutrino density at redshift z in h^2 Msun/Mpc^3. 
         */
         function rho_nu(z){

            return Onu0 / Om0 * rho_m(z);
        ...
        function createPkTable(args){
            ...
            for (let i = 0, lnk = lnka; i < n; i++, lnk += dlnk){
                ...
                let k  = Math.exp(lnk);
                let tk = transfer(k, 0);
                let Dk = Math.pow(k, ns + 3) * tk * tk * ONE_2SQRPI; // unnormalised dimensionless power  
        ...
        /**
         * Parse the overdensity value.
         */
        function parseOverdensity(value, z){

            if (value === "vir"){

                let delta = 18 * Math.PI**2;

                // corrections 

                let x = Om(z) - 1;
                if (flat){
                    delta += 82.0*x - 39.0*x**2
                }
                else if (Math.abs(Ode0) < 1e-08){
                    delta += 60.0*x - 32.0*x**2;
                }

                delta = Math.round(delta);

                return Math.round( delta * rhoCritical(z) / rho_m(z) );
            }

            let delta = parseInt(value.slice(0, value.length-1));
            let ref   = value.slice(value.length-1, value.length);
        ...
        var publicAPI = {
                            ...
                            parseOverdensity           : parseOverdensity,
                            massFunction               : massFunction,
        ...
        /**
         * Returns the peak height of density fluctuations.
         * 
         * @param x Input argument - could be a mass, radius or variance value.
         * @param z Redshift.
         * @param arg Specify the input argument. 'm' for mass, 'r' for radius and 
         *        's' for variance.
         */
        function peakHeight(x, z, arg){
            ...
            switch (arg) {
                case "m":
                    y = variance( lagrangianR(x), z ); 
                    return DELTA_C / Math.sqrt(y);
                ...
                default:
                    throw CosmoError("CosmoError", "invalid key: '" + arg + "'.");
            }
        }
    ...
    /** Constants */
    const constants = {
                        RHO_CRIT0_ASTRO: RHO_CRIT0_ASTRO,
                        DELTA_C        : DELTA_C,
                        MPC            : MPC,
                        C_SI           : C_SI,
                        YEAR           : YEAR,
